...
public class MrPrinterTypeController {
	...
	public String showMrPrinterType(Model model,Integer id,Integer currentPage,Integer pageSize,String name,Integer hospitalId){
		...
		return "basic/updatemrprinttype";
	}
	...
	/*@ResponseBody
	@RequestMapping("/validateMrPrintTypeIsUse")
	public Result validateMrPrintTypeIsUse(Integer id){
		boolean result = false;
		if(id!=null){
			result = mrPrinterTypeService.validateIsUse(id);
		}else{
			System.err.println("验证打印类型是否被引用所需的编号为空");
		}
