	...
	<div style="width: 100%; height:100%;">
		...
		<div style="width:100%; height: 100%;">
			<div id="list_NR" style="float: left;">
				<div class="loc">
					<h3>全国版RC020-ICD-10诊断编码管理</h3>
				</div>

				<div class="list_con_table">
					<input type="hidden" value="${basicDataQuery.totalCounts}" id="totalCountsHidden">
					<form action="${path}/diagnosis/getDiagnosisRc020Icd10s" method="get" id="queryForm">
						<div class="search_table">
							<ul>
								...
								<li class="Label_2" style="width:10%;margin-top: -1%">
								<button class="button" type="submit">查询</button></li>
							</ul>
						</div>
					</form>
					<form action="${path}/diagnosis/getDiagnosisRc020Icd10s" method="get" id="pageForm">
...
<script type="text/javascript">
	   function deleteBasic(code){
	            		 confirm("删除提示","是否删除？",function(){
	            			 $.ajax({
	 	         	            type: "POST",
	 	         	           url:"<%=path%>/diagnosis/deleteStdDiagnosisRc020Icd?code="+code,
	 	         	            success: function(data) {
	 	         	            	if (data.success==true) {
	 	         	            		location.href="<%=basePath%>/diagnosis/getDiagnosisRc020Icd10s";
	 	         	    			} else {
	   ...
	   function insertBasic(){
		   location.href="<%=basePath%>/diagnosis/addDiagnosisRc020Icd10Page";
	   }
	   ...
	   function updateBasic(code){
		   ...
		   location.href = "<%=basePath%>/diagnosis/showStdDiagnosisRc020Icd10?code="+code+"&currentPage="+currentPageCount
