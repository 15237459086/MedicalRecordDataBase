		...
		<div style="width:100%; height: 100%;" id="content">
			<div id="list_NR" style="float: left;">
				...
				<div class="list_con_table">
					...
					<form action="${path}/exportType/getMrPrinterTypes" method="get" id="queryForm">
					...
					<form action="${path}/exportType/getMrPrinterTypes" method="get" id="pageForm">
				...
				<table class="tableList" cellpadding="0" cellspacing="0">
					...
					<tbody id="tbody">
						<c:forEach var="basic" items="${basics}">
							<tr>
								<td class="tdLabel_5" style="display: none">${basic.printTypeId }</td>
								...
								<a onclick="updateBasic(${basic.printTypeId})">
								<img src="${path}/static/images/edit.png" style="height: 23px;margin-bottom: 8px;"/>
								</a></td>
								<td class='tdLabel_5' style="text-align: center">
								<a onclick="deleteBasic(${basic.printTypeId})">
...
<script type="text/javascript">
	   function deleteBasic(id){
		   <%--  $.ajax({
	           type: "POST",
	           url:"<%=path%>/basic/validateMrPrintTypeIsUse?id="+id,
	            success: function(data) {
	            	if (data.success==true) {   --%>
	            		 confirm("删除提示","是否删除？",function(){
	            			 $.ajax({
	 	         	            type: "POST",
	 	         	           url:"<%=path%>/exportType/deleteMrPrinterType?id="+id,
	 	         	            success: function(data) {
	 	         	            	if (data.success==true) {
	 	         	            		location.href="<%=basePath%>/basic/getMrPrinterTypes";
	 	         	    			} else {
	   ...
	   function insertBasic(){
		   location.href="<%=basePath%>/exportType/addMrPrinterTypePage";
	   }
	   ...
	   function updateBasic(id){
		   ...
		   location.href = "<%=basePath%>/exportType/showMrPrinterType?id="+id+"&currentPage="+currentPageCount+"&pageSize="+pageCount+"&name="+queryName+"&hospitalId="+hospitalId;
	   }
	</script>
</html>