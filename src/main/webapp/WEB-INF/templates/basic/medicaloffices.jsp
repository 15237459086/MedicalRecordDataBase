		...
		<div style="width:100%; height: 100%;">
			<div id="list_NR" style="float: left;">
				...
				<div class="list_con_table">
					...
					<form action="${path}/basic/getMedicalOffices" method="get" id="queryForm">
						<div class="search_table">
							<ul>
								...
								<li class="Label_2" style="margin-top: -1%">
								<button class="button" type="submit">查询</button></li>
							</ul>
						</div>
					</form>
					<form action="${path}/basic/getMedicalOffices" method="get" id="pageForm">
...
<script type="text/javascript">
	   function deleteBasic(id){
		   $.ajax({
	           type: "POST",
	           url:"<%=path%>/basic/validateMedicalOfficeIsUse?id="+id,
	            success: function(data) {
	            	if (data.success==true) {
	            		 ...
	            		 confirm("删除提示","是否删除？",function(){
	            			 $.ajax({
	 	         	            type: "POST",
	 	         	           url:"<%=path%>/basic/deleteMedicalOffice?id="+id,
	 	         	            success: function(data) {
	 	         	            	if (data.success==true) {
	 	         	            		location.href="<%=basePath%>/basic/getMedicalOffices";
	 	         	    			} else {
	   ...
	   function insertBasic(){
		   location.href="<%=basePath%>/basic/addMedicalOfficePage";
	   }
	   ...
	   function updateBasic(id){
		   ...
		   location.href = "<%=basePath%>/basic/showMedicalOffice?id="+id+"&currentPage="+currentPageCount+"&pageSize="+pageCount+"&name="+queryName+"&hospitalId="+hospitalId;
	   }
	</script>
</html>