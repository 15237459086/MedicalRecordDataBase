$(function(){
	...
	var submitData = "tableName=hospital";
	$.ajax({
		url: basePath + "basic/ajax_query_basic_encoding",
		type: "GET",
		dataType: "json",
		data:submitData,
		success: function( data ) {
			...
			if(success){
				var hospitals = data['data']['queryDatas'];
				if(hospitals.length > 0){
					$("select[name$='hospitalCode']").each(function(){
						var options = "";
						for(var index in hospitals){
							var hospitals = hospitals[index];
					    	options+="<option value='"+hospitals.uniq_code+"'>"+hospitals.label+"</option>";
					    } 
				       $(this).append(options);
